...
import { Row, Spinner, Pagination, Button, Alert } from "react-bootstrap";
...
import OpenPost from "../component/OpenPost";
...
function Home() {
  ...
  const handleOpenPostClose = () => setShowPostOpen(false);
  ...
  return (
    <div className="mt-5">
      ...
      {postData.errorMessage && (
        <Alert variant="danger">{postData.errorMessage}</Alert>
      )}
      ...
      <OpenPost
        show={showPostOpen}
        post={openPost}
        handleClose={handleOpenPostClose}
        onLike={onLike}
        onUnlike={onUnlike}
      />
